...
## Bootstrap Rust 放置护栏（待 Gate 6 收敛）

下列名称只用于 Bootstrap 阶段避免业务规则散落，并表达当前依赖方向；它们不是最终 bounded context、crate 清单、部署单元或数据库所有权。Gate 6 可以合并、改名或重新分配。DISC-001 完成前不得因为这里存在一个名称，就创建对应业务模块、表或大规模移植代码。
