...
//! always travels with the source kind, so the reader can tell an author's own sentence from a
//! reader's comment from text a machine read off an image.
...
/// ranks above comments: someone searching a phrase that appears inside an image wants to be
/// told that, and the slot key carried back lets the Inspector jump straight to it.
///
...
/// prefers the author's own body text, then a reader's comment, and only falls back to
/// machine-read text when neither exists, preferring a body image over a cover.
///
...
pub(crate) async fn read(
    ...
    if let Some(query) = query.map(str::trim).filter(|value| !value.is_empty()) {
        ...
        if let Some(fragment) = matched_comment(tx, item, query, as_of).await? {
            return Ok(Some(fragment));
        }
    ...
    if let Some(fragment) = leading_comment(tx, item, as_of).await? {
        return Ok(Some(fragment));
    }
...
/// A reader comment containing the query.
///
/// Comments are deduplicated to their current version the same way every other comment read in
/// this crate is: latest observation per external id, restricted to accepted packages. The
/// author is never carried into the excerpt — the list shows the sentence, not who said it.
async fn matched_comment(
    tx: &mut Transaction<'_, Postgres>,
    item: &MaterialLibraryItem,
    query: &str,
    as_of: &str,
    ...
    let row = sqlx::query(
        "WITH current_comment AS ( \
           SELECT DISTINCT ON (comment.comment_external_id) comment.material_ref,comment.body_text,comment.comment_external_id \
         ...
         ) SELECT material_ref,body_text FROM current_comment \
           WHERE lower(COALESCE(body_text,'')) LIKE '%' || lower($3) || '%' \
           ORDER BY comment_external_id LIMIT 1",
    )
    .bind(item.identity.public_ref)
    .bind(as_of)
    .bind(query)
        ...
        body.as_deref()
            ...
            .map(|hit| {
                fragment_from(
                    ...
                    "comment_body",
                    "SEARCH_MATCH",
                    row.get::<Option<Uuid>, _>("material_ref"),
                    None,
                )
            })
    }))
...
/// The most substantial reader comment, used when nothing else is readable.
///
/// The platform's like count is not retained per comment, so "most substantial" is measured as
/// the longest root comment. Length is a weak proxy and is not presented as popularity anywhere
/// in the UI; the tie-break on external id keeps the choice stable between reads.
async fn leading_comment(
    tx: &mut Transaction<'_, Postgres>,
    item: &MaterialLibraryItem,
    as_of: &str,
) -> Result<Option<MaterialEvidenceFragment>, sqlx::Error> {
    let row = sqlx::query(
        "WITH current_comment AS ( \
           SELECT DISTINCT ON (comment.comment_external_id) comment.material_ref,comment.body_text,comment.is_reply,comment.comment_external_id \
           FROM linggan_material_comment comment \
           JOIN linggan_runtime_capture_package package USING(package_ref) \
           WHERE comment.content_public_ref=$1 AND package.accepted_at <= $2::timestamptz \
           ORDER BY comment.comment_external_id,comment.observed_at::timestamptz DESC,comment.created_at DESC,comment.material_ref DESC \
         ) SELECT material_ref,body_text FROM current_comment \
           WHERE body_text IS NOT NULL AND length(btrim(body_text)) > 0 \
           ORDER BY is_reply,length(btrim(body_text)) DESC,comment_external_id LIMIT 1",
    )
    .bind(item.identity.public_ref)
    .bind(as_of)
    .fetch_optional(&mut **tx)
