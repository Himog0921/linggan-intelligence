...
pub async fn read_comment_research(
    ...
    if cursor
        .as_ref()
        .is_some_and(|c| c.text != query.text || c.work_ref != query.work_ref)
    {
        return Err(CommentResearchReadError::InvalidQuery);
    }
    ...
    let mut rows = sqlx::query(
        "SELECT current.*,true AS is_current FROM linggan_comment_research_current($1::timestamptz) current
         ...
         WHERE ($2::uuid IS NULL OR current.content_public_ref=$2)
         AND strpos(lower(COALESCE(current.body_text,'')),lower($3))>0
         AND ($4::uuid IS NULL OR current.material_ref>$4) ORDER BY current.material_ref LIMIT 21")
        .bind(&as_of).bind(query.work_ref).bind(&query.text).bind(cursor.map(|c| c.after))
        .fetch_all(&mut *tx).await?;
    let more = rows.len() > 20;
    rows.truncate(20);
    let items: Vec<_> = rows.iter().map(source_row).collect();
...
pub async fn read_comment_research_context(
    ...
    Ok(
        ...
        "work":work_context,"workUrl":format!("/api/local/work-resources/{}", source.work_ref)}),
    )
}
