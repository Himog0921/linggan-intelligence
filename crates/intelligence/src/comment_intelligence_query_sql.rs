...
WITH config AS (
 ...
 COALESCE(cl.state,'pending') AS clean_state,
 ...
 LEFT JOIN linggan_ci_source_research h ON h.canonical_ref=s.canonical_ref AND h.domain_ref=s.domain_ref
 ...
 LEFT JOIN clean_latest cl ON c.is_own_domain AND cl.source_ref=s.source_ref
 ...
 WHERE ($7::uuid IS NULL OR s.work_ref=$7)
 AND ($6::text='' OR strpos(lower(COALESCE(s.body,'')),lower($6))>0)
 ...
 md5(count(*)::text||COALESCE(bit_xor(hash_record_extended(ROW(canonical_ref,source_sha256,analysis_ref,revision,likes,analysis_state),0))::text,'')||COALESCE(bit_xor(hash_record_extended(ROW(canonical_ref,source_sha256,analysis_ref,revision,likes,analysis_state),1))::text,'')||COALESCE((SELECT string_agg(problem_ref::text||revision::text,',' ORDER BY problem_ref) FROM linggan_ci_problem WHERE domain_ref=(SELECT domain_ref FROM config)),'')||COALESCE((SELECT string_agg(term||revision::text,',' ORDER BY term) FROM linggan_ci_term_setting WHERE domain_ref=(SELECT domain_ref FROM config)),'')||COALESCE((SELECT max(updated_at)::text FROM linggan_ci_projection_cursor c JOIN source s USING(canonical_ref)),'')||COALESCE((SELECT revision::text FROM linggan_ci_rule_release WHERE domain_ref=(SELECT domain_ref FROM config)),'')) AS revision FROM filtered
), paged AS (
 SELECT f.*,CASE WHEN f.body IS NOT NULL THEN f.body WHEN f.domain_ref=(SELECT domain_ref FROM observation_domain WHERE is_own_domain) THEN (SELECT m.body_text FROM linggan_material_comment m WHERE m.material_ref=f.source_ref) ELSE (SELECT x.body_text FROM cross_industry_comment x WHERE x.comment_ref=f.source_ref AND x.domain_ref=f.domain_ref) END AS display_body FROM (
...
SELECT jsonb_build_object(
 ...
 'workTitle',work_title,'creatorDisplayName',creator_display_name,'labels',all_labels,'problemRefs',problem_refs,'cleanState',clean_state,
 ...
 (array_agg(DISTINCT f.source_ref))[1:1000] refs,(array_agg(jsonb_build_object('sourceRef',f.source_ref,'body',left(f.body,180),'likes',f.likes) ORDER BY f.likes DESC NULLS LAST,f.canonical_ref))[1] representative
