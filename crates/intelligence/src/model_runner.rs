...
async fn reserve_call(db: &Database) -> Result<Option<ReservedCall>, ModelError> {
    ...
    sqlx::query("UPDATE linggan_comment_analysis_work SET state=CASE WHEN attempts<3 THEN 'pending' ELSE 'failed' END,lease_ref=NULL,lease_until=NULL,failure_code='lease_expired' WHERE state='running' AND lease_until<=scope_001_now() AND work_ref IN(SELECT work_ref FROM linggan_comment_model_work)").execute(&mut *tx).await?;
