...
impl ResearchPacket {
    ...
    fn rejected_field_diagnostic(
        ...
        let (group, n) = relative
            .split_once('[')
            .map(|(g, n)| (g, n.trim_end_matches(']').parse::<usize>().unwrap_or(0)))
            .unwrap_or(("labels", 0));
        let field = &item[group][n];
        let path = format!("{base}.{relative}");
        let issue = if code == "field_schema_invalid" {
            schema_issue(
                &schema["properties"]["comments"]["items"]["properties"][group]["items"],
                Some(field),
                &path,
            )
        } else {
        ...
        let envelope: Value = match serde_json::from_str(text) {
            ...
            Err(error) => {
                return vec![diagnostic(
                    ...
                    "json_invalid",
                    "$",
                    "返回完整JSON对象，不夹带Markdown",
                    json!({"type":"invalid_json","bytes":text.len(),"line":error.line(),"column":error.column(),"category":if error.is_eof(){"incomplete"}else{"syntax"}}),
                )];
            }
        };
