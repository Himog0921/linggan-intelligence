...
pub async fn read_model_settings_with_plan(
    ...
    Ok(
        ...
        "models":models.iter().map(|r|json!({"modelRef":r.get::<Uuid,_>("model_ref"),"modelId":r.get::<String,_>("model_id"),"connectionVersionRef":r.get::<Uuid,_>("connection_version_ref"),"connectionName":r.get::<String,_>("connection_name"),"currentVersion":r.get::<bool,_>("current_version"),"commentQualified":r.get::<bool,_>("qualified"),"testState":r.get::<Option<String>,_>("test_state"),"enabled":r.get::<bool,_>("enabled"),"test":r.get::<Option<Value>,_>("test")})).collect::<Vec<_>>(),"config":config,
...
pub async fn current_comment_model_state(db: &Database) -> Result<Value, ModelError> {
    let row=sqlx::query("SELECT c.enabled,config.config_ref,COALESCE((SELECT i.state='succeeded' AND i.result->>'commentContract'=$1 AND i.result->>'commentQualified'='true' FROM linggan_model_invocation i WHERE i.model_ref=m.model_ref AND i.operation='probe' ORDER BY i.created_at DESC LIMIT 1),false) AS qualified FROM linggan_model_workspace ws JOIN linggan_model_config config ON config.config_ref=ws.default_config_ref JOIN linggan_model_entry m USING(model_ref) JOIN linggan_model_connection_version v ON v.version_ref=m.connection_version_ref JOIN linggan_model_connection c USING(connection_ref) WHERE ws.singleton").bind(crate::comment_daily::DAILY_RULE).fetch_optional(db.pool()).await?;
    Ok(match row {
        Some(r) => {
            json!({"modelConnected":r.get::<bool,_>("enabled") && r.get::<bool,_>("qualified"),"commentContract":crate::comment_daily::DAILY_RULE,"commentQualified":r.get::<bool,_>("qualified"),"modelState":if !r.get::<bool,_>("enabled"){"PAUSED"}else if r.get::<bool,_>("qualified"){"CONFIGURED"}else{"NEEDS_QUALIFICATION"}})
        }
        None => {
            let availability=sqlx::query("SELECT count(*) AS models,count(*) FILTER(WHERE c.enabled) AS enabled,count(*) FILTER(WHERE c.enabled AND COALESCE((SELECT i.state='succeeded' AND i.result->>'commentContract'=$1 AND i.result->>'commentQualified'='true' FROM linggan_model_invocation i WHERE i.model_ref=m.model_ref AND i.operation='probe' ORDER BY i.created_at DESC LIMIT 1),false)) AS qualified FROM linggan_model_entry m JOIN linggan_model_connection_version v ON v.version_ref=m.connection_version_ref JOIN linggan_model_connection c USING(connection_ref) WHERE v.revision=(SELECT max(latest.revision) FROM linggan_model_connection_version latest WHERE latest.connection_ref=c.connection_ref)").bind(crate::comment_daily::DAILY_RULE).fetch_one(db.pool()).await?;
            let state = if availability.get::<i64, _>("qualified") > 0 {
                "NEEDS_SELECTION"
            ...
            json!({"modelConnected":false,"modelState":state})
        }
    })
}
