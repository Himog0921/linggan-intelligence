...
const prompt=JSON.stringify({contract:'comment-research.semantic.v1',task:'Return JSON.',outputSchema:schema,untrustedMaterial:{comments:[]}});
...
function complete(res,api,text='{"outcome":"no_signal","reason":"synthetic"}') {
...
test('actual SDK sends V1 schema for analysis and JSON mode for DeepSeek chat',async()=>{
  await fixture(async bodies=>{
    for(const api of ['openai-responses','openai-completions']){
      assert.equal((await execute({...request,api,operation:'analyze'})).ok,true);
      const body=bodies.at(-1);
      if(api==='openai-responses'){
        assert.deepEqual(body.text.format,{type:'json_schema',name:'comment_research_semantic_v1',schema});
...
test('official supported OpenAI models receive the V1 schema in each documented protocol',async()=>{
  await fixture(async bodies=>{
    for(const api of ['openai-responses','openai-completions']){
      assert.equal((await execute({...request,baseUrl:'https://api.openai.com/v1',modelId:'gpt-4o-mini',api})).ok,true);
      const body=bodies.at(-1);
      if(api==='openai-responses')assert.deepEqual(body.text.format,{type:'json_schema',name:'comment_research_semantic_v1',schema});
      else assert.deepEqual(body.response_format,{type:'json_schema',json_schema:{name:'comment_research_semantic_v1',schema}});
    }
  });
});
test('V1 problem resolution receives its distinct named schema',async()=>{
  const resolutionPrompt=JSON.stringify({contract:'comment-research.semantic.v1/problem-resolution',task:'Return JSON.',outputSchema:schema,untrustedMaterial:{candidates:[]}});
  await fixture(async bodies=>{
    assert.equal((await execute({...request,prompt:resolutionPrompt})).ok,true);
...
test('V3 research packets retain structured output on the documented DeepSeek protocol',async()=>{
  const semanticV3=JSON.stringify({contract:'comment-research.semantic.v3',task:'Return JSON only.',outputSchema:schema,examples:[{outcome:'no_signal',reason:'synthetic'}],untrustedMaterial:{comments:[]}});
  const resolutionV3=JSON.stringify({contract:'comment-research.semantic.v3/problem-resolution',task:'Return JSON only.',outputSchema:schema,examples:[{decision:'new_problem'}],untrustedMaterial:{candidates:[]}});
  await fixture(async bodies=>{
    assert.equal((await execute({...request,prompt:semanticV3})).ok,true);
    assert.equal(bodies.at(-1).text.format.name,'comment_research_semantic_v3');
    assert.equal((await execute({...request,prompt:resolutionV3})).ok,true);
    assert.equal(bodies.at(-1).text.format.name,'comment_research_problem_resolution_v3');
  });
});
test('V4 research packets retain structured output on the documented DeepSeek protocol',async()=>{
  const semanticV4=JSON.stringify({contract:'comment-research.semantic.v4',task:'Return JSON only.',outputSchema:schema,examples:[{outcome:'no_signal',reason:'synthetic'}],untrustedMaterial:{comments:[]}});
  const resolutionV4=JSON.stringify({contract:'comment-research.semantic.v4/problem-resolution',task:'Return JSON only.',outputSchema:schema,examples:[{decision:'new_problem'}],untrustedMaterial:{candidates:[]}});
  await fixture(async bodies=>{
    assert.equal((await execute({...request,prompt:semanticV4})).ok,true);
    assert.equal(bodies.at(-1).text.format.name,'comment_research_semantic_v4');
    assert.equal((await execute({...request,prompt:resolutionV4})).ok,true);
    assert.equal(bodies.at(-1).text.format.name,'comment_research_problem_resolution_v4');
  });
});
test('V5 research packets retain structured output on the documented DeepSeek protocol',async()=>{
  const semanticV5=JSON.stringify({contract:'comment-research.semantic.v5',task:'Return JSON only.',outputSchema:schema,examples:[{outcome:'no_signal',reason:'synthetic'}],untrustedMaterial:{comments:[]}});
  const resolutionV5=JSON.stringify({contract:'comment-research.semantic.v5/problem-resolution',task:'Return JSON only.',outputSchema:schema,examples:[{decision:'new_problem'}],untrustedMaterial:{candidates:[]}});
  await fixture(async bodies=>{
    assert.equal((await execute({...request,prompt:semanticV5})).ok,true);
    assert.equal(bodies.at(-1).text.format.name,'comment_research_semantic_v5');
    assert.equal((await execute({...request,prompt:resolutionV5})).ok,true);
...
test('unknown host, path, model and unrelated prompts do not inherit structured capability',async()=>{
  await fixture(async bodies=>{
    for(const change of [
      ...
      {prompt:'Return JSON.'},
      {prompt:JSON.stringify({contract:'another-contract',untrustedMaterial:{contract:'comment-research.semantic.v1',outputSchema:schema}})},
    ]){
...
test('invalid server schema is rejected before transport; provider schema rejection is not retried',async()=>{
  await fixture(async bodies=>{
    assert.equal((await execute({...request,prompt:JSON.stringify({contract:'comment-research.semantic.v1',outputSchema:{type:'array'}})})).failureCode,'invalid_request');
