(() => {
  ...
  const errorText = { invalid_model_command:'输入不符合模型设置要求。', model_revision_conflict:'设置已变化，请刷新后重试。', model_disabled:'连接未启用。', model_not_qualified:'请先完成模型能力测试。', model_database_unavailable:'模型设置数据库暂不可用。' };
  ...
  async function submitDialog(event){event.preventDefault();if(state.busy)return;state.busy=true;try{if(!$('#model-command').reportValidity())return;await saveConnection(event.submitter=== $('#dialog-test'));}catch(error){$('#dialog-feedback').textContent=error.message;}finally{state.busy=false;}}
