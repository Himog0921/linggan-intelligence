(() => {
  ...
  const state = {view:initial === 'queries' ? 'queries' : params.get('view') || 'voices',text:params.get('text') || '',workRef:params.get('workRef') || '',cursor:null,history:[],next:null,collectionRef:params.get('collectionRef') || '',generation:0,detailGeneration:0,collections:[]};
  ...
  function renderAssets(data) {
    state.next=data.nextCursor;$('result-count').textContent=`${data.total.toLocaleString()} 次人工收存`;
    $('results').innerHTML=data.items.length ? table([['收存表达','46%'],['为什么收存','32%'],['来源资格','22%']],data.items.map(a=>`<tr><td><blockquote>${esc(a.quote || '来源当前不可读')}</blockquote></td><td>${esc(a.reason || '来源受限，停止展示衍生内容')}<div class="lgi-research-meta">${esc(a.createdAt)}</div></td><td>${a.eligibility==='READABLE'?`${a.isCurrentSource?'当前来源版本':'固定历史来源版本'}<br><button type="button" data-source="${a.sourceRef}">查看来源</button>`:'来源不可用'}<div class="lgi-research-meta">人工收存不代表已证实</div></td></tr>`)) : empty('这个范围还没有收存表达。从原声浏览中选中一句有价值的话，写下理由即可保存。');
  }
  ...
  function renderQueries(data) {
    state.next=null;$('result-count').textContent=`${data.items.length} 个查询`;
    $('results').innerHTML=data.items.length ? table([['查询名称','35%'],['保存条件','45%'],['使用','20%']],data.items.map(q=>{
      ...
      return `<tr><td>${esc(q.name)}</td><td>${esc(q.text || '全部可读原声')}<div class="lgi-research-meta">${q.workRef?'限定作品':'跨全部作品'} · 每次使用重新查询最新可读材料</div></td><td><a href="/corpus/comments?${esc(p)}">打开查询</a></td></tr>`;
  ...
  $('research-command-form').addEventListener('submit',async event=>{
    ...
    try {
      ...
      if(state.view==='assets')await load();
