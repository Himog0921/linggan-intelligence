...
proof_database="linggan_comment_study_proof_${proof_suffix}"
proof_container="linggan-comment-study-proof-${proof_suffix}"
proof_volume="linggan-comment-study-proof-${proof_suffix}-data"
...
[[ "$proof_container" =~ ^linggan-comment-study-proof-[a-zA-Z0-9_-]+$ ]] || exit 1
[[ "$proof_volume" =~ ^linggan-comment-study-proof-[a-zA-Z0-9_-]+-data$ ]] || exit 1
...
RUST_TEST_THREADS=1 cargo test -p linggan-intelligence --test comment_study_rebuild_postgres --locked -- --ignored --nocapture --test-threads=1
printf '%s\n' 'Comment Study rebuild PostgreSQL proof passed'
