...
proof_database="linggan_agent_proof_${proof_suffix}"
proof_container="linggan-agent-proof-${proof_suffix}"
proof_volume="linggan-agent-proof-${proof_suffix}-data"
proof_user="agent_proof_admin"
...
[[ "$proof_database" =~ ^linggan_agent_proof_[a-zA-Z0-9_]+$ ]] || { echo "unsafe proof database name" >&2; exit 1; }
[[ "$proof_container" =~ ^linggan-agent-proof-[a-zA-Z0-9_-]+$ ]] || { echo "unsafe proof container name" >&2; exit 1; }
[[ "$proof_volume" =~ ^linggan-agent-proof-[a-zA-Z0-9_-]+-data$ ]] || { echo "unsafe proof volume name" >&2; exit 1; }

if ! docker info >/dev/null 2>&1; then
  echo "Agent PostgreSQL proof was not started: the Docker daemon is unavailable." >&2
  exit 1
fi
...
cleanup() {
  ...
  [[ "$cleanup_failed" -eq 0 ]] || { echo "Agent proof cleanup failed" >&2; exit 1; }
  echo "Agent PostgreSQL proof cleanup verified; isolated container and volume were removed"
  exit "$task_exit"
}
trap cleanup EXIT
...
cargo test -p linggan-intelligence --test agent_runtime_postgres --locked -- --ignored
echo "Agent Candidate PostgreSQL proof passed"
