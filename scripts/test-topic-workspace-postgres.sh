...
proof_database="linggan_topic_proof_${proof_suffix}"
proof_container="linggan-topic-proof-${proof_suffix}"
proof_volume="linggan-topic-proof-${proof_suffix}-data"
proof_user="topic_proof_admin"
...
[[ "$proof_database" =~ ^linggan_topic_proof_[a-zA-Z0-9_]+$ ]] || { echo "unsafe proof database name" >&2; exit 1; }
[[ "$proof_container" =~ ^linggan-topic-proof-[a-zA-Z0-9_-]+$ ]] || { echo "unsafe proof container name" >&2; exit 1; }
[[ "$proof_volume" =~ ^linggan-topic-proof-[a-zA-Z0-9_-]+-data$ ]] || { echo "unsafe proof volume name" >&2; exit 1; }

if ! docker info >/dev/null 2>&1; then
  echo "Topic PostgreSQL proof was not started: the Docker daemon is unavailable." >&2
  exit 1
fi
...
cleanup() {
  ...
  [[ "$cleanup_failed" -eq 0 ]] || { echo "Topic proof cleanup failed" >&2; exit 1; }
  echo "Topic PostgreSQL proof cleanup verified; isolated container and volume were removed"
  exit "$task_exit"
}
trap cleanup EXIT
...
cargo test -p linggan-api --bin linggan-api --locked topic_import_and_read_compose_exact_work_resources_without_copying_them -- --ignored
echo "Topic Workspace PostgreSQL proof passed"
