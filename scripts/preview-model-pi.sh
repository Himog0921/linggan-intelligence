...
cleanup() {
  ...
  [[ -z "${preview_fixture_pid:-}" ]] || kill "$preview_fixture_pid" 2>/dev/null || true
...
export LINGGAN_MODEL_SYNTHETIC_PREVIEW=SYNTHETIC-NOT-EVIDENCE
fixture_log="$(mktemp /tmp/model-pi-fixture.XXXXXX)"
"${LINGGAN_PI_NODE:-$HOME/.nvm/versions/node/v24.13.0/bin/node}" "$project_root/apps/pi-adapter/test/fixture-server.mjs" >"$fixture_log" 2>&1 &
preview_fixture_pid=$!
for _ in {1..30}; do [[ -s "$fixture_log" ]] && break; sleep 0.1; done
LINGGAN_LOCAL_DATABASE_URL="$preview_url" "$project_root/target/debug/linggan-comment-worker" --execute >"${preview_log}.worker" 2>&1 &
preview_worker_pid=$!
...
preview_api_pid=$!
printf 'Synthetic preview: http://127.0.0.1:%s/settings/models\n' "$preview_port"
printf 'Synthetic provider URL: %s; credential marker: SYNTHETIC-NOT-A-CREDENTIAL; model: synthetic-good\n' "$(head -n 1 "$fixture_log")"
printf 'Worker PID: %s; fixture PID: %s\n' "$preview_worker_pid" "$preview_fixture_pid"
printf 'API PID: %s; API log: %s\n' "$preview_api_pid" "$preview_log"
printf 'Isolated container: %s; volume: %s\n' "$proof_container" "$proof_volume"
printf '%s\n' 'SYNTHETIC / NOT EVIDENCE. This process owns the isolated preview. Stop this script to remove its API, comment worker, fixture, container and volume.'
