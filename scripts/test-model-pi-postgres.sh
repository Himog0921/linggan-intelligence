...
if ! docker info >/dev/null 2>&1; then
  echo "Model Pi PostgreSQL proof was not started: the Docker daemon is unavailable." >&2
  exit 1
fi
...
cleanup() {
  ...
  [[ "$cleanup_failed" -eq 0 ]] || { echo "Model Pi proof cleanup failed" >&2; exit 1; }
  echo "Model Pi PostgreSQL proof cleanup verified; isolated container and volume were removed"
  exit "$task_exit"
}
trap cleanup EXIT
...
printf '%s\n' 'Model Pi PostgreSQL proof passed'
