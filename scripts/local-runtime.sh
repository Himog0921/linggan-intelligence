...
verify_application_database_credentials() {
  ...
  # the password works for the application connection path.
  if ! docker compose run --rm --no-deps -T -e "PGPASSWORD=$POSTGRES_PASSWORD" postgres \
    psql -X -v ON_ERROR_STOP=1 -h postgres -U "$POSTGRES_USER" -d "$database_name" -c 'SELECT 1;' >/dev/null; then
...
repair_password() {
  ...
  # just verified from this invocation's .env explicitly; never select the
  # container's startup environment as the repair authority.
  docker compose exec -T -e "LINGGAN_REPAIR_PASSWORD=$POSTGRES_PASSWORD" postgres sh -ceu '
    case "$LINGGAN_REPAIR_PASSWORD" in
      ""|*[!0-9a-f]*)
        echo "POSTGRES_PASSWORD must be the hex local secret generated by setup-local-env.sh" >&2
        exit 1
        ;;
    esac
    printf "ALTER ROLE linggan_dev_admin PASSWORD '\''%s'\'';\\n" "$LINGGAN_REPAIR_PASSWORD" \
