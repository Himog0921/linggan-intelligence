...
只能取回原授权。`0.8.52` 已删除旧 Workbench 控制面、旧远程内容处理器、旧本地同步表和
...
失败连坐。`0.8.53` 把页面采集器的 epoch 毫秒观察时间在持久缓存边界规范为 RFC3339；服务端
若仍明确拒绝某个 immutable Package，插件只为该 task 追加 `capture_delivery_rejected` 终态，
