...
const { createLocalProducerOutbox } = await import('../src/linggan/localProducerOutbox.js');
const { createLocalAttempt, createLocalSubmission, createManualTaskSpec } = await import('../src/linggan/adapter.js');

test('manual TaskSpec remains flat and scheduler is never fabricated', () => {
  const spec = createManualTaskSpec({ taskId: '11111111-1111-4111-8111-111111111111' });
  assert.deepEqual(spec, {
    contractVersion: 'linggan.task-spec.v1',
    taskId: '11111111-1111-4111-8111-111111111111', source: 'manual', platform: 'xhs',
    pageType: 'search_results', target: 'current_visible_search_surface',
    capabilitiesRequested: ['discover_visible_cards'], maximumQuota: 20,
