...
> 事实来源: 已确认 Gate 2–3 边界、当前 Rust/PostgreSQL/插件代码与测试、已部署 Browser Producer `0.8.29`、候选 `0.8.30` 发行包、工位与真实 Package/Receipt/Materialization 回执
...
- `0.8.30` 候选补上一个严格位于 **Attempt 之前** 的恢复接缝：任务已领取但页面窗口/标签页/就绪检查失败时，插件仅上传受控的本机失败码与幂等 `failureRef`；服务端追加 dispatch-failure 审计并原子把同一个 frozen TaskSpec 退回 `pending`，一分钟后再由服务端派发。它不新建 Attempt、Package、Receipt、Evidence 或来源错误正文，也不生成第二份任务。未持有 live claim、安装已换代或 failure identity 冲突时，服务端拒绝重排而不是猜测恢复。
