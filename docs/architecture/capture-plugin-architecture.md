...
> 事实来源: 已确认 Gate 2–3 边界、当前 Rust/PostgreSQL/插件代码与测试、已部署 Browser Producer `0.8.32`、候选 `0.8.33` 发行包、工位与真实 Package/Receipt/Materialization 回执
...
- 真实 `0.8.32` 已领取 `author_profile` task，但作者页等待阶段连续回报 `page_timeout` 并保留同一任务的可审计重排。`0.8.33` 不再把 Chrome 的顶层 `complete` 当作页面读取的前提；它只在 URL 经静默期且同 URL 的 content runtime 对 `getPageContext` 回应时开始采集。未注入、重定向或 URL 不一致仍保持失败回退，不从等待成功推导 Evidence。
