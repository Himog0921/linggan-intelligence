...
[`plans/active/arc-001-architecture-closure-decision-map.md`](plans/active/arc-001-architecture-closure-decision-map.md) 的 `product-shell` 与 `primary-daily-job` 已 resolved：Linggan Intelligence 是独立且完整的 Web 产品；每天先让 Mog 处理少量值得关注事项，理解来源、出现原因与不确定性，再决定暂不处理、继续观察、进入 Topic 深入或提出行动。内容工作台只是历史原型和能力样本，不是运行时依赖；首页五个名称是产品职责，不是五个已授权运行实例。Mog 已进一步确认「今日关注」是唯一默认入口，并选择首批真实 Canary 方案 B：同步纳入受限媒体。该确认只解锁 P0、Capture 与媒体责任的文档收口；不表示 Web、真实 producer、插件、媒体下载、OCR/转录、对象存储、Agent 或真实 Canary 已获实施授权。当前 [`architecture/capture-control-contract.md`](architecture/capture-control-contract.md) 为待独立审查的草案，明确部分合格结果可入受限链、Coverage 限制解释但不连坐材料价值，并把真实账号、配额、预算、风控与恢复等选择保留为 `DECISION_REQUIRED`。
...
| 6 | ARC-001 | 产品与系统架构收口决策图 | 进行中（合同草案/独立审查） | Mog 已确认 P0 默认入口与含受限媒体的 Canary 方向；P0、Capture 与媒体合同草案仍须审查、集成，并由新的真实 Canary SCOPE 收口后才可申请真实执行 |
