...
ARC-001 已同步为“已回答问题不再重复提问”：Capture Control Contract 已由 PR #17 合入；媒体卡改为 V2 语义、PR #15 草案、当前产品规则与 Rust 实现的主线校准；第一阶段运行时只剩当前实现缺口基线，首个用户可见范围只剩正式 SCOPE 冻结。Issue #10 和 #14 保持开放；旧 Issue #11/#16/#18 与 PR #12/#13/#15/#19 将按“已吸收/已被取代”收口。关闭旧卡不证明真实媒体、OCR/ASR、长期调度、部署或业务验收。
