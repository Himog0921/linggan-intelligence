...
### COMMENT-RESEARCH-PROBLEM-RESOLUTION-V2 / Issue #285（源码实施推进；不触及运行时）

Mog 已确认把 Problem 从单条 Atom 的标题生成改为可审计的长期归并合同。DEC-0004 规定：模型只能输出有来源的 Problem Frame 与固定语义维度比较；程序验证候选/引用/版本并决定 assigned、deferred、not-user-problem 或 out-of-scope。无候选匹配的单条信号作为可读待归并保留；只有两个独立、eligible、等价的信号与一个受其共同支持的最小定义才可创建 Problem。当前候选源码已完成 ADHD scope、V7 Frame/closed-world candidate 合同、`deferred_*` 可读面，以及 pair checkpoint、受限 pair worker、catalog guard、账本关联和 lease recovery；同作者、同来源或原文完全重复的信号不能构成新建依据。定向隔离 PostgreSQL proof 已确认两条独立 `deferred_novel` Atom 会在一个事务中生成一个稳定 Problem、两条 membership 并推进 catalog revision，同时拒绝同作者 pair；规则内核 59/59 与定向 API/UI 检查通过。完整 harness 的 Docker 端口读取已改为结构化 inspect 并成功启动 42 项 PostgreSQL suite，随机容器和卷已清理；但本轮交互未保留最终退出回执，故完整 suite 仍不得标绿。当前工作从 `main@dab0be3` 的专属 worktree 开始，复用父评论语境、V6 输出合同、WeMM candidate recall 与 #281 execution history；不重建评论研究、不清理旧 Problem、不运行真实模型、不应用共享 migration、不刷新 `:3000`、不启用排程或合并。
