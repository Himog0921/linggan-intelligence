...
ARC-001 的 `first-producer-canary` 已由 Mog 选择 B：首批真实 producer Canary 必须同步纳入受限媒体。决策材料见 [`architecture/first-producer-canary-decision-pack.md`](architecture/first-producer-canary-decision-pack.md)。该决定只解锁 `capture-control-contract` 与 `media-lifecycle-contract` 两项独立设计工作，并规定它们均为真实 Canary 的前置门；它不授权真实 producer、账号、插件、媒体下载、OCR/转录、schema、存储、保留/删除政策、Web、Agent 或运行时实现。`p0-surface-prototype` 仍等待 Mog 的走查确认；`DESIGN-002` 的 Topic Reference Page 只是其可走查输入，而不是完整答案。首页职责到运行时的语义已澄清，但不授权 P0 或运行时实现。`ADV-AUDIT-001` 的独立复核最终处置见 [`audits/pre-implementation-architecture-audit-2026-08-21.md`](audits/pre-implementation-architecture-audit-2026-08-21.md)；它没有改动 F01 migration/合同，也不授权真实访问。
