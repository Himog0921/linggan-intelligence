...
并行的产品决策下一步仍在 [`plans/active/arc-001-architecture-closure-decision-map.md`](plans/active/arc-001-architecture-closure-decision-map.md) 的 `p0-surface-prototype` 与 `first-producer-canary`；两票均已解锁但尚未获得 Mog 的最终回答。`first-producer-canary` 现有一份草案决策材料 [`architecture/first-producer-canary-decision-pack.md`](architecture/first-producer-canary-decision-pack.md)，它建议文本与结构优先，但明确保持 `DECISION_REQUIRED`，不授权真实 producer、媒体、插件、Web、Agent 或旧系统迁移实现。`DESIGN-002` 的 Topic Reference Page 是 `p0-surface-prototype` 的一个可走查输入，而不是这张票的完整答案。首页职责到运行时的语义已澄清，但不授权 P0 或运行时实现。`ADV-AUDIT-001` 的独立复核最终处置见 [`audits/pre-implementation-architecture-audit-2026-08-21.md`](audits/pre-implementation-architecture-audit-2026-08-21.md)；它没有改动 F01 migration/合同，也不授权真实访问。
