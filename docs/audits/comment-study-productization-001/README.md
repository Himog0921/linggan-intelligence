...
- [P1作品目录与共享标题](p1-work-catalog.md)：本次 `/works` 后端、Evidence标题复用、目录共享资格与未连接UI边界。
...
| P1 材料可查 | 清洗缓存、评论目录/详情/历史、批次计数、作品分页后端候选 | 完整作品语境、原冻结来源资格统一、有界cache tick、用户评论和作品选择UI、真实接口/SQL验收 | 实施中，未验收 |
...
| P5 集成与发布包 | 已有合成测试候选与T01–T54清单 | 编译/单元/隔离PG/并发/历史保留/迁移/性能/浏览器，主线整合、文档治理及发布前核验 | NOT_RUN；不具备发布结论 |
