...
| `DEV-06` 语义分析与受控 AI Agent | AI 能在固定材料、权限、预算和版本下分类、聚类、找反例并提出候选，不自行制造正式知识或扩采 | 离线合同切片执行中 | #112 exact Pack 与 #113 A1–A2 合成/脱敏 spike 已授权；真实阶段仍等待 DEV-05、材料权限和 provider 条件 | 确定性分析与 Agent 解释分责；输出有引用/反例/未知；工具逐次授权；候选不自动发布；真实评测证明有用 | [`architecture/agent-architecture.md`](architecture/agent-architecture.md)、[`architecture/agent-candidate-analysis-contract.md`](architecture/agent-candidate-analysis-contract.md)、[`audits/pi-agent-kernel-upstream-assessment-2026-08-20.md`](audits/pi-agent-kernel-upstream-assessment-2026-08-20.md) | PostgreSQL/deterministic/Pi transport seam 仅为 Draft；真实 Pi/provider 与真实材料仍未授权/验证 |
...
- Topic Workspace 与单一 A1–A2 Agent Runtime 只在交付 Draft 中实现；Corpus、Signal、Claim/Brief、语义聚类、完整 Web 工作台仍未实现；
- Pi 仅有固定版本 transport seam 与合成 fixture，不是已安装、已运行或已验证 provider 的 Agent 内核；
