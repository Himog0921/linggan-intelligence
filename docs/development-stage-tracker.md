...
| `DEV-03` 真实 producer 事实审计 | 知道插件在真实搜索页、作者页、详情和评论场景究竟能看到什么、缺什么、何时停止 | 受限探针执行中 | 完整 DEV-03 仍等待 DEV-02 与对应 Capture/处置合同；但 Mog 已于 2026-08-26 对 Issue #74 授权最小、只读、无接入的页面字段探针 | 完成低风险探查；字段、时间、身份、终态、Coverage、账号差异和风控报告有真实证据；形成受限 Capture Contract | [`plans/active/aud-xhs-001-xiaohongshu-capture-probe-refresh.md`](plans/active/aud-xhs-001-xiaohongshu-capture-probe-refresh.md)、[`plans/active/arc-001-architecture-closure-decision-map.md`](plans/active/arc-001-architecture-closure-decision-map.md)、[`architecture/capture-plugin-architecture.md`](architecture/capture-plugin-architecture.md) | #74 只允许匿名化字段/计数/状态探针；不提交或持久化真实材料，不使 DEV-03、Canary 或插件接入自动通过 |
