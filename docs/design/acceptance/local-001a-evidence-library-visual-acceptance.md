...
- LIDS 强度 / Pattern / Token 依据: L1 Corpus Explorer + embedded L2 Inspector; `lids_tokens.css` is the single editable 107-token runtime source, with `tokens.md` as its versioned verification mirror; the Rust test compares every token name and value, and page CSS only consumes it
