...
- 已检查的响应式/可访问性条件: 2026-08-25 根据独立视觉审查的 P1 发现，重新使用独立 Chrome 对 local route 真实截图并测量：1280×800 得到 128px 页头、216px rail、420px inspector，整个 document 为800px高，Results 为235px内部滚动、Inspector 为235px内部滚动；1440×900 得到 128px 页头、216px rail、420px inspector，document 为900px高，二者各416px内部滚动；1920×1080 得到 128px 页头、216px rail、440px inspector，document 为1080px高，二者各596px内部工作面；390×844 的顶层导航会换行，页头实测为182px（131px top row + 51px context row），shell/rail 实测从 y=182 开始，因而不覆盖 context row，随后才是 375px 可用内容宽度的 rail → workspace → inspector 与页面原生滚动。桌面不存在 document 向下延展或横向内容溢出；每一处均检出 `SOURCE_INCOMPLETE` 和 `NO_ACCEPTED_MATERIAL_AVAILABLE`，未检出 V7 模拟运行态、计数、示例内容或动作结果；21 个未接通控件带 `aria-disabled`。页面保留 `nav`/`aria-current`、`main`/`aside`/heading 语义。
