...
- 真实已有 DeepSeek 连接：单次固定合成材料 probe，v3 结构与引用通过，3902ms / 输入503 / 输出202 token；随后保存默认配置，回执 `SAVED_NO_ANALYSIS_STARTED`。3000 浏览器确认 DeepSeek 默认选择与通过状态。没有发送真实评论或启用每日计划。
- 交付分支：隔离 PostgreSQL 每日研究 19 项通过，含新增配置状态生命周期回归（未选默认可 probe、v2 拒绝、失败回执不放行、停用/恢复不丢当前连接版本）。隔离容器和卷已清理。
- 新界面：合成 HTTP 环境使用实际 HTML/JS，浏览器走通旧合同提示 → 单弹窗测试 → 选择为默认 → 显式保存 → 返回评论研究链接。此环境没有真实凭据或模型调用。
- 广域十万条性能用例本次失败：原声热 P95 1260ms，超过既有1200ms阈值；其余3项查询证明通过。未更改性能阈值，不将此作为配置回归失败或性能通过声明。
- 新页面代码尚未合并/部署；3000 本次只更新真实模型校验回执和默认配置。真实评论分析质量仍待用户确认批次后验收。

编译与静态检查：`cargo check -p linggan-api --locked`、4 份改动 JavaScript 的语法检查、`git diff --check` 与项目治理检查通过。3000 评论 API 返回 `CONFIGURED / modelConnected=true`，分析调用仍为0。
