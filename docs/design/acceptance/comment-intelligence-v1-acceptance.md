...
| T | 已删除的旧 `comment_intelligence_statistics.rs` / `comment_intelligence.rs`：`compare` 与上列四个确定性测试；曾提供实际窗口、范围、版本与聚合 |
...
| E | 已删除的旧 `test_comment_intelligence_evaluation.py`：合成成绩不获真实资格、作品泄漏／重复预测、引用错误、pairwise／拒绝全部、版本不可比、独立群体审查、CLI／输入覆盖保护、来源声明／Unicode／重复 JSON key，共八项 |
| UI | 已删除的旧 `comment_intelligence.js` / `comment_intelligence.css`：`query`、`navigate`、`load`、`renderOverview`、`voiceTable`、`renderProblems`、`renderDaily` 等旧页面入口 |
| 内部职责拆分 | 已删除的旧问题命令、模型候选复验与词项/向量候选模块；当前 V1 不通过 re-export 保留旧 API |
| API／状态 | 已删除的旧 `comment_intelligence.rs`、`comment_daily.rs`、人工命令、问题索引、日批运行；历史 `0048` migration 文件继续保留，但不提供旧 runtime 路径 |
...
浏览器集成、HTTP 检查和每日批次的这一段是 2026-09-08 的历史回执。其 `scripts/verify-comment-runtime-api.mjs`、batch/request/context 路由以及日批读取模型，均已由 COMMENT-RESEARCH-RESET-001 terminal cutover 删除；它们不再是可执行命令或当前验收入口。
