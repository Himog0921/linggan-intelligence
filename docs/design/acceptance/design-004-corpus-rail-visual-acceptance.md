...
| 面层分层 | 分辨 rail 与主工作面是两个层 | rail 是次级面 | 灰底 + 双层点阵，与上方上下文行纹理垂直连续 | 无动作 | 通过 |
| 移动折叠 | 390 宽下 rail 转为全宽区块 | 结构降级但语义不变 | 纹理固定 84px 不随块宽等比放大；重音不失控 | 无动作 | 通过 |
...
- 已批准的设计规则: DESIGN-003 纯白基线、线条六原则、粗野重音位移受限允许、L1 点阵测量场
...
| 前端实现 | VERIFIED | 真实 Chrome 三视口实拍 | 未覆盖 1536/1728/1920/2560 与 430×932 |
...
- 发现的规格冲突: 线条原则 2 要求「已有底色差异处不再补线」，与 V7 三栏骨架的 2px 结构线存在字面冲突。本次按结构级线宽保留，并记为可复核例外
