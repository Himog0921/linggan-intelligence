...
| 多候选来源 / Live Photo partial | 核验槽位来源组、地址断言、组件和组合关系 | 本次 Package 只有一个 slot origin group/generation；其 declared Bundle 下分 still/motion；每条断言具 `candidateRef/order/primary/sourceField/observedAt/expiresAtState/expiresAt`；下载尝试绑定精确 `candidateRef` 并显示时间、terminal、failureReason | still acquired / motion failed / overall partial；primary 只作当次建议；候选顺序不推断组件；历史代次不混入本次 Package | 无媒体请求 | VERIFIED：第六作品行与 Media Inspector |
...
- 最新截图：桌面/窄屏位于 `/tmp/linggan-evidence-reference.contract-final/desktop-default-1440x900.png` 与 `/tmp/linggan-evidence-reference.contract-final/mobile-default-390x844.png`。两张均已人工查看，不提交 Git；此前 error/processing 专项截图仍为同轮设计验收的辅助资产，但本次合同修订以最新两张为准。
...
- 六个作品 option 均独立显示作者 lane；评论和回复在讨论组内仍保留各自状态，未被压成单一讨论状态。结果使用 `role=listbox` + `role=option` 和 roving `tabindex`，任一时刻只有当前可见 option 为 `tabindex=0`；ArrowUp/ArrowDown、Home/End、Enter/Space 均可切换当前作品和焦点。
...
- 第六作品验证本次 `package/synthetic-006` 对 `slot/live-photo/body/01` 只有一个槽位级 `origin-group/slot-006/g3`；declared Bundle 再关联 `still_image` 与 `motion_stream`。still A/B 与 motion M1 均逐条显示 `candidateRef/order/primary/sourceField/observedAt/expiresAtState/expiresAt`；`primary` 明确只是 Producer 当次建议。两个下载尝试分别绑定 `candidate/still/A` 与 `candidate/motion/M1`，并显示 startedAt、endedAt、terminal 以及失败尝试的 failureReason。候选顺序不推断组件，跨代历史不混入本次 Package。
- 普通未选中作品行 hover 只改变背景并增加内侧结构线，不使用 transform 或 4px hard shadow；当前选中行保持冻结，不被 hover 改写。
