...
此前已经进入 `monitoring` 或 `paused`、但**缺少合格搜索面基线**的历史关键词不构成例外：它们的「建立档案」必须能创建**修复性**第一阶段 `deep_archive` 工单；已有搜索面但欠详情时只能走作品作用域的补详情，不能重扫搜索面。这不会恢复或新建巡查，直到同一完成判据确认两段建档完成。下文保留的“观察中的词不能建档”描述只记录返修前的事实，不能作为当前验收或实现依据。
