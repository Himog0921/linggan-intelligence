...
> 状态: 权威当前
> 最后核对: 2026-09-22
> 适用范围: `http://localhost:3000/collection/*` 的六个子面
...
- 五秒主动作: 管理领域和 Target 用途 / 建立档案 / 查看任务 / 查看档案 / 补采缺口 / 处理异常；不把“重建目录”“继续完善”“查看进度”作为日常产品概念。
...
| 序 | 路由 | 中文名 | 唯一主问题 |
|---|---|---|---|
| 01 | `/collection/attention` | 待处理 | 哪些事情真正需要人介入？ |
| 02 | `/collection/targets` | 观察目标 | 我们长期正在观察谁 / 什么？ |
| 03 | `/collection/operations` | 生产流 | 系统此刻怎样运行、刚刚观察到了什么？ |
| 04 | `/collection/tasks` | 采集任务 | 具体有哪些执行任务、到哪一步？ |
| 05 | `/collection/runtime` | 执行工位 | 执行工位、队列、租约与回执是否健康？ |
| 06 | `/collection/domains` | 领域管理 | 正式研究哪些领域、各自观察什么、是否接受新工作？ |

运行态内含 `?mode=now|trace|review` 三个模式，均为真实地址。

**已记录的产品保留意见**：`DESIGN-005` 的第一性审核认定，执行任务与执行运行时两个子面对本阶段唯一用户（产品负责人本人）没有决策价值——它们服务的是排查故障的场景。本次保留原五个子面的既有职责与当前 rail 顺序，并在末尾新增领域管理作为第六个子面；该意见留待 Mog 决定是否重组，不由实现者自行合并。
...
| `DOMAIN_UNASSIGNED` | 候选 Observation Target 尚未关联任何 Domain | 目标身份已保存；需先由人把它关联到一个或多个 active Domain 并分别选择 role | 已有材料用途；可以直接建立档案；系统替人决定研究领域 |
...
| 管理 Domain 与 Target 用途 | 受控写入 + 明确选择 | 同一 Target 可关联多个 Domain、每个 Domain 单独选择 primary/reference；只影响今后的准入，不重采作者、不复制 Evidence、不改写冻结用途 |
...
| 彻底删除观察目标 | 受控写入 + 确认 | 先读取真实预览并要求输入确认名；删除目标控制面（含详情会话、授权尝试、lane preparation、执行资格）及只对该目标目录生效的人工失效结论，并逐项披露数量。任何仍被有效 Domain 用途或安装级明确风险页信号引用的控制关系都必须安全处理；作品、详情、评论、Package 和 Receipt 始终保留。 |
