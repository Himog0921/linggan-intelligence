...
本补充只收口新 rebuild 页面已经拥有的「选择作品并创建不外发 Run」操作面；不改变
StudyRun、policy、模型、来源、权限或任一 API 字段。

- 页面视觉强度与主 Pattern 保持 L1 / Corpus Explorer。`h1` 保留为读屏标题，视觉上由
  Context Bar 与工作区内的操作栏承担定位，不再建立 page-local Hero。
- 首屏顺序固定为：紧凑的研究策略工具栏、作品候选表、运行/Signal/Problem 的读数区。
  标题、说明和连接状态不得以大块空白或介绍卡抢占候选表的可视高度。
- 候选作品是同构、最多 100 条的真实数据，使用连续表格而非卡片网格。表头固定；桌面端
  表体在工作区内滚动，小屏回退为自然页面滚动。列只陈述选择、作品和已知的可研究评论数。
- 筛选仅在浏览器已取得的 `eligibleWorks` 集合内执行，必须清楚标记为「筛选已加载作品」；
  它不是新的服务器搜索、范围扩张或重新选择研究输入。全选只作用于当前筛选结果。
- `selected-count`、可见行 checkbox、表头全选和创建 Run 的启用状态消费同一选择集合；筛选
  不得清空已选择但暂时不可见的作品，也不得把可见行数误写为已选数。
- 运行、Signal、Problem 的紧凑读数仍只读取原有只读 endpoint。没有 Run、Signal 或 Problem
  时分别说明其真实空态，不能用装饰性空白或三张固定高度卡片掩盖。
...
本补充的验收还必须确认：在 1440px 桌面视口，进入页面后无需先越过页面介绍即可看到
作品表的表头与至少一行候选；在有 100 条候选时可以筛选、选择、滚动表体并保持选择数；
在窄屏不强制嵌套滚动。
