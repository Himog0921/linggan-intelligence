...
> 2026-09-07 语义收敛补充：本清单中任何“生命周期散点”“建立标准目录”“继续完善”“查看建档状态”表述均由 `OBSERVATION-TARGET-SEMANTIC-CLOSURE-001` 取代。日常界面只使用建立档案、查看任务、查看档案、补采缺口、处理异常；creator 为概览/作品/巡查，keyword 为概览/命中作品/巡查。

> 2026-09-15 显示口径更新：Mog 明确要求首层“作品目录”仅展示已接纳作品数量，例如 `201 篇`；历史目录不删除，也不得在这个数字格拼接“· 边界未知”。该展示收敛不改变目录基线、详情分母、巡查资格或将该数量扩展为平台全量的合同。
...
- “作品目录”是 count-only 数值格：任何可显示目录一律写为 `N 篇`，不在数字后附加范围或边界状态文字。
- 头像占位也不使用主动换行或“物化中”等工程语言；使用单字首字母、默认头像或“暂无头像”。
- 档案状态来自当前目录根的去重目录、初始目录 Coverage、详情、作者关联问题、隔离问题和 durable Work 状态；已接纳且详情完整的 target-scoped 历史目录显示“已有目录”和真实 `详情 / 目录`，但不把该数量说成平台全量。未证明“200 上限或表面末端”且未形成完整既有目录时显示“待建标准目录”，不从按钮点击或 lifecycle 名称猜在途。
...
| 初始目录在途或已有任务待处理 | 查看建档状态 | 打开档案 Tab，显示真实 Work/Lease/回执状态，不重复提交，也不把待处理写成执行中 |
| 初始目录因风险、时间、配额或执行故障提前停止，且当前无在途 | 建立标准目录 | 保留旧 WorkOrder/Package 为历史；重新发起一次标准主页目录建立，不把旧部分目录当作详情分母 |
| 基线可用、已知作品缺详情 | 继续完善 | 复用首次建档的 canonical root，每次只派发不超过 3 篇详情，不重扫主页目录 |
| 有未处理关联、隔离或详情读取受阻问题 | 查看档案问题 | 打开真实问题/缺口，不声称已修复；读取受阻不是页面不存在 |
...
2. 首次动作创建当前 canonical root；Request、Admission、根 WorkOrder、`progressiveArchive.version=1` marker 与 Lease 在同一事务形成，根范围固定为 `maximumQuota=200`。当旧根未证明目录边界且已无在途时，“建立标准目录”保留旧历史并建立新的当前根；它不是把 200 伪装成数据版本号。已接纳且详情完整的历史目录继续显示为当前在库事实，并可接收后续巡查增量。
...
6. “继续完善”只复用当前已证明的 canonical root；旧 Authorization 仍有效时继续使用，过期后只接受新的同目的、同目标类型且上限不小于 200 的有效 Authorization。每次冻结不超过 3 篇已知缺详情 Work，不重新派发 `author_profile` 或 `profile_discovery`，不重扫主页。
7. 每篇固定详情、最多 30 条评论、回复展开最多 2 次；媒体及 OCR/ASR 只在当前有效授权已允许时进入该小批次。已成功、完整且精确归属到该 creator Target 的巡查发现会并入当前目录集合，因而增加作品目录与详情分母；无新作品的合格巡查增加成功时间但不增加数字。
...
11. 同一当前 root 中同一冻结作品详情连续三次 `page_read_failed` 后，页面显示“详情读取受阻”。它停止该作品的自动重试与同页后续 lane，不把它改写为页面不存在，也不把它计入详情、Attempt、Package、Receipt 或 Evidence；同批其他作品仍可继续。
...
- 档案: 只有“作品目录”“已取得详情”“当前可分析”三项数字读数；目录与详情必须来自同一当前目录根。旧根未证明边界时前两项显示状态而不写数；问题以独立缺口文案/动作表达，“语料页”和深层材料名称不冒充统计值。
