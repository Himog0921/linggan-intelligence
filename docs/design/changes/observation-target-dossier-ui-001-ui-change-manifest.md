...
- 档案状态只来自当前目录根的去重目录、初始目录 Coverage、详情、作者关联问题、隔离问题和真实在途执行；未证明“200 上限或表面末端”的历史目录显示“目录待重建”，不从按钮点击或 lifecycle 名称猜在途。
...
| 初始目录因风险、时间、配额或执行故障提前停止，且当前无在途 | 重建目录 | 保留旧 WorkOrder/Package 为历史；重新发起一次标准主页目录建立，不把旧部分目录当作详情分母 |
...
2. 首次动作创建当前 canonical root；Request、Admission、根 WorkOrder、`progressiveArchive.version=1` marker 与 Lease 在同一事务形成，根范围固定为 `maximumQuota=200`。当旧根未证明目录边界且已无在途时，“重建目录”保留旧历史并建立新的当前根；它不是把 200 伪装成数据版本号。
