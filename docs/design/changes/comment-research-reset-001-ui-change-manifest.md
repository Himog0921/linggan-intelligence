...
- 事项：Issue #274；将评论研究增长型表格组织为固定的可视研究工作区，而不是一条不断向下生长的文档。
- 表面：桌面端的页头、运行状态和 Tab 占用上部；用户原声、用户问题、运行记录在剩余空间内保持上下文、范围控制和底部分页可见，表格本身内部滚动，列表标题固定。小屏幕仍使用自然页面滚动。表格上方与末尾保留同一分页控制；页面不引入吸顶浮层、新导航或新数据对象。
- 行为与边界：两个位置操作同一 `limit` / `offset`、URL 页码和现有读取接口；只改变当前读范围，不创建 Run、修改研究数据、调用模型或影响调用账本。
- 验收：正常 100% 缩放下，用户问题页面在首屏表格列标题前能看到“显示 1–10，共 N 条”和下一页；底部分页不再需要浏览器缩放或整页滚动才能抵达；表格纵向滚动时列标题仍在表面顶部。
