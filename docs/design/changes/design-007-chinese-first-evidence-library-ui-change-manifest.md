...
- 最终语言收口只修补两个 Evidence 页面内 header 字段：成功读投影的服务状态为「本机服务 / 已接纳发现材料」，`LOCAL HOST / ACCEPTED DISCOVERY` 仅以紧邻 `.v7-tech-key` 旁注保留；顶栏时区为「本机时区」，`UTC+08` 同样仅作紧邻技术键。本补正不修改 `shell.rs`、共享 shell、API、数据库、查询、采集、Coverage 或任何状态判定。
