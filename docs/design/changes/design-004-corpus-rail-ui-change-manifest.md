...
- 用户可见结果（经 2026-08-26 修订）: rail 保持纯白面，与主工作面继续以 2px 结构线分隔；选中项保持实心墨面，新增自右边缘向左离散衰减的像素纹理、实色硬阴影与 `translate(-2px,-2px)` 位移；导航序号由 9px 提升到 11px；hover 改为白实面浮起
...
| 变更 | 变更前 | 变更后 | 规则依据 |
|---|---|---|---|
| ~~rail 面~~ | `--v7-white` | ~~`--v7-gray` + 双层点阵~~ | 已于 2026-08-26 回滚，见 §9 |
| 导航项常态 | 自带白底 | `transparent` | 与白面等效；保留以便未来换面时不必再改 |
| hover | 底色变 `--v7-gray` | 保持 `--v7-gray` + 序号转 signal | 序号转 signal 为本次新增 |
...
## 9. 2026-08-26 修订 · 面层回滚

Mog 在实际页面上判定：rail 复用与上下文行完全相同的点阵纹理后，原本独一份的「上下文层」不再特殊，页面层次被压平。按设计来源优先级第 1 层（用户最新确认），面层部分回滚。

| 项 | 08-25 实现 | 08-26 修订后 | 说明 |
|---|---|---|---|
| rail 底 | `--v7-gray` + 56/14px 双层点阵 | `--v7-white` | 纹理重新成为上下文行独占的标识层 |
| 导航项 hover | 白实面浮起 | `--v7-gray`（回到原行为） | 白面上「白色浮起」不成立 |
| 栏脚上边线 | `--v7-line-strong` | `--v7-line` | 白底上 hairline 已足够 |
| 选中态重音 | 像素纹理 + 硬阴影 + 位移 | **保留** | Mog 未提出异议；白底上重音反差更强 |
| 序号 11px / ghost / focus-visible / reduced-motion | 已实现 | **保留** | 均有独立规则依据，与面层选择无关 |

**这次回滚说明了什么**：`primitives.md` 线条原则 2「面代替线」在本页不能无条件套用——当页面已存在一条以纹理承担语义的横向带时，再给纵向面用同款纹理，减少的线条不抵消失的层次。该观察记入 `lids/migration-log.md`，供后续判断是否需要在 primitives 中补充「纹理的唯一性」约束；在补充之前，它只是本页的一次实测结论，不构成新规则。

**验证**：`cargo test -p linggan-api` 8 passed / 5 ignored；1440×900 真实 Chrome 实拍复核。
