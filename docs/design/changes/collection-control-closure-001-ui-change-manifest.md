...
### 2026-09-10 首单观察启动与缺失身份摘要诊断

- Runtime 的“观察账号”为 `—` 时，必须区分两件事：**尚未观察**不会阻断首单；而 `identity_key_missing` 明确表示本机没有配置账号身份摘要密钥，认证身份 observation 不会被服务端接收，但明确登录墙、冷却或访问限制仍可上报并阻断后续接活。两种状态都不显示身份摘要、原始账号标识、Cookie 或任何秘密。
- 已观察但未人工确认的账号显示为可确认候选，不能被文案写成“必须绑定才能接活”；只有 `changed`（已有绑定与新身份不一致）才是账号变更阻断。
- Runtime 用 `data-account-observation-availability` 暴露 `ready` 或 `identity_key_missing` 的受限诊断合同，供受控验收读取；它不是新的设置入口，也不提供生成或编辑密钥的 UI。
