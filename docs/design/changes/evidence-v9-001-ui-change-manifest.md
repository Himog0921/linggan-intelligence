...
LIDS 语义护栏写着「L1/L2 最大 1440px；超宽屏增加外白而非拉宽正文」。此前研读行的内容列没有任何上限：实测 1920 视口 + 标准检查器时，内容列 968px、一行原声引文横跨 915px，行看起来被拉长且右侧留下大片空洞。现在内容列上限 760px，材料轨仍右对齐——沿列扫读时比值成列，这是表格式列表该有的样子。表格与封面布局不受该上限约束（它们本来就是按列宽排的）。

封面网格的最小列宽由 196px 提高到 248px：1920 下由 8 列收到 6 列，卡片元信息不再折成三行。
