...
- 新增 `crates/evidence/src/material_evidence_fragment.rs`：为每个作品读取**一条有界原声摘录**，随摘录返回来源类型（正文 / 图片文字 / 视频画面文字 / 转录）、选取依据、是否截断、来源引用与命中槽位。评论正文不属于普通列表读取；它只经本机授权的评论研究通道按页返回。
- 新增响应字段 `items[].evidenceFragment`。`null` 是真实答案，表示这个作品还没有可引用的文本，页面按诚实空态渲染，不用标题顶替。
- 摘录上限 88 字符，命中时保留 20 字符前文；每个偏移量都是**字符**偏移，不是字节偏移。
- 取材顺序：有检索词时 正文 → 图片文字/转录（命中优先，且携带 `slotKey` 供 GO TO MATCH 定位）；无检索词时 正文 → 正文图片文字。即使评论匹配检索词，也不把评论正文带入普通列表响应。
...
| 反馈 | 处理 |
|---|---|
| 搜索框选中后多一层外框 | 焦点改为在原框内表达：内描边 1px + `FIND` 标记反白。不再叠加 outline，键盘可达性由这两处共同承担 |
| 筛选浮层与其中的下拉不是本项目风格 | 浮层阴影由柔性 `shadow-overlay` 换成硬阴影 `shadow-brutal`；四个原生 `<select>` 全部换成自绘 listbox（原生弹层由系统绘制，圆角、蓝底高亮与阴影都改不掉）。嵌套浮层互不误关，`Esc` 先关内层 |
| 页头整块可删，作品数放到左栏「证据库」上 | `.ev-page-head`（任务陈述 + 说明 + 四项读数）整体删除；作品数改为左栏条目右侧的红色角标，其余三项读数留在角标的 `title` 里 |
| 检查器头部标题与限制行可删 | 标题降为 `sr-only`（选中行两列之外就写着），限制行移入概览事实网格。头部只剩宽度/关闭控件、动作按钮与 ref |
| 字号取上一版与当前的中间值 | 全表回调：行标题 18→16.5px、原声 15→13.5px、行内元信息 12→10.5px、事实网格 12.5→11.5px、Tab 14→12.5px |

另修：自绘下拉此前把「有值」当成「已筛选」，导致默认值本身就带值的读取窗口永远显示激活红条；现按「是否偏离默认」判断，筛选计数同源。

### 材料轨与查看器（第三轮）

- **媒体卡宽度减半**：标准档 `clamp(180px,74%,300px)` → `clamp(90px,37%,150px)`，加宽与专注档同比例。同一屏能比对的对象数翻倍，序号角标与元信息随之缩小、元信息改为两行堆叠以适应更窄的卡片。
- **查看器支持缩放**：滚轮以指针为锚点缩放、双击在贴合与 250% 之间切换、放大后可拖拽平移、`+` `-` `0` 快捷键，头部有 −／百分比／+ 三键（百分比即复位）。平移量按实际溢出夹紧，图片不会被拖出视野后找不回来。切换对象与关闭时缩放复位。
- **弹出图加画框**：边框挂在外层画板而不是图片本身——图片一旦可缩放拖动，边框长在图片上就会跟着滑出画面。画板 2px 墨色边 + 硬阴影，`overflow:hidden` 负责裁切，与本系统其余浮层同一套硬边界语言。

### 查看器尺寸与检查器头部（第四轮）

- **查看器不再把图片撑满**。画板改为 `min(56vw,760px) × min(76vh,900px)`，一屏完整看完一个对象即可；需要看细节时用缩放，而不是让每张图默认占满整屏。窄屏另给一档 `min(74vw,520px) × min(64vh,620px)`。
- **上一张／下一张贴在画板两侧**。此前它们钉在窗口最左右边缘，离图片一两百像素；现在紧邻画板，连着翻几张不用把手移开，翻页过程留在同一个沉浸视图里，画板尺寸不随对象变化。
- **检查器头部并成一行**：打开原文、补采、作品 ref、宽度三键与关闭全部同一行、同一控件高度（32px）。头部高度由约 130px 降到 97px；这块面板本来就是高度最紧张的地方，不该拿第二行放控件。
