...
2026-09-07 同包 DeepSeek 输出上限修正：沿用原读取回执和 L1 状态/交互分类，只改 model-settings 弹窗与模型行的截断反馈。保持成功/生成响应/评论资格分开、历史回执不重写、无新组件或 token。上游默认思考与当前纯文本合同通过显式请求参数对齐；数据库、HTTP/SDK 和浏览器证明见原验收记录新增节。

## 2026-09-08 评论模型配置返修（#169）

Claim：用户本轮明确要求解决评论研究与模型设置之间的配置阻塞；根代理单人执行，`codex/comment-model-readiness` / `.worktrees/comment-model-readiness`。

- 表面：供应商行、同一编辑测试弹窗、默认选择表单、评论研究状态与确认弹窗；不新建 Tab 或视觉组件。
- 状态：未配置、未测试、旧合同待重测、当前测试失败、当前校验通过待选择、已配置、停用；资格始终来自最新成功调用与当前合同，失败不能放行。
- 依赖：现有模型读取 API 与默认保存合同、评论研究页面；复用已有 L1 表单 / L2 原生弹窗、按钮和中文提示，不改 token、采集或模型算法。
- 验收：隔离 PostgreSQL 覆盖未选默认也能测试、旧合同不合格、当前成功可保存、失败/停用不放行；前端检查清晰下一步、成功选择后仍须保存、保存后返回评论研究、未合格不能发起批次。
- 实测：现有 DeepSeek 仅发送一条固定合成评论，v3 通过（3902ms，输入503 / 输出202 token）。截图为 v2 历史失败；没有真实语料分析。本次不降低引用与结构验证，不启用每日观察。


## MODEL-CALL-004 配置调用能力与评论格式诊断分离

授权来源：Mog确认排查结论后“认可，按照这个进行修复”。范围：供应商行、单配置弹窗、默认模型选择/保存、评论确认/日程入口；不新增Tab、全局token或导航。根负责probe安全诊断与合同，backend代理统一保存/读取/预检/派发门槛，UI代理按modelCallable呈现并做行为回归。

验收：成功调用且评论partial/failed可保存；最新失败/截断/未测/停用不可；旧成功回执无明细时如实解释；诊断安全转义和未知键不泄露；保存不启动；正式评论依旧拒绝错误引用；旧合同计划不复活。维护测试scripts/tests/model_callability_ui.mjs，结果记入同项活跃计划。


MODEL-CALL-004 验证：38 项 DOM 通过；当前真实 shell/HTML/CSS/JS 在本地合成 API 下进行 headless 浏览器检查，1280/900 两视口的部分字段状态、展开诊断、选择默认、保存均通过，零脚本错误。/tmp/model-call-ui-partial-{1280,900}.png、/tmp/model-call-ui-default-900.png 与 /tmp/model-call-ui-headless.log 为合成 UI 证据；根已看图复核 900 视口的滚动区和底部操作栏。无共享 3000 发布或真实模型验收。
