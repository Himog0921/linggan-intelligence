...
> 事实来源: Mog 2026-09-15 的规格、真机尺寸候选页与选型回复（候选 B）、`EVIDENCE-COVER-FLIP-CARD-001`、`EVIDENCE-COVER-CARD-PROPORTION-002`、LIDS、现行 `evidence_library.js/css`
...
- 用户可见结果：一排仍是 6 张卡。三处表面不再靠黑框区分，而是靠底色层级区分（页面白 → 卡面 `#eef0f1` → 舞台纯白 → 铭牌 `#f7f8f8`）；卡面多一层极淡的干燥纸面颗粒；舞台点阵真正铺满 3:4 区域并回到规格的 8px / 1px；选中对象仍是整卡 Signal 橙，但橙色面本身有材质；标题回到系统字阶而不是页面大标题字阶。
...
| 卡面材质 | 给「印刷卡」以纸面密度，不参与表达任何状态 | LIDS-MAT-001 的材料纪律（一个容器至多一种、正文背后不铺纹理）；颗粒本身不在其六态内，见 §3 的 DECISION_REQUIRED | 方格纹理、噪点图、状态或质量暗示 |
...
- 材质纪律：一个容器至多一种材质。卡面颗粒只挂在 `.ev-cover-face`，且被 `.ev-cover-face-inner` 的内容层覆盖，因此标题、正文、数据永不被纹理压住；舞台点阵只挂在 `.ev-cover-stage-frame--diagram`。两者图案不同（非共线散点 20px vs 均匀点阵 8px），不会读成同一种图案。
- 取值纪律（如实列出写死值）：本包新增的值里，除既有 `--lgi-*` token 外只有两组，都是材质密度参数——卡面颗粒（点半径 `.6px`、周期 `20px`／选中态 `18px`、三个散点的偏移 `13px 7px` / `6px 15px` / `12px 6px` / `6px 14px`）与舞台点阵（半径 `1px`、周期 `8px`，来自用户规格「8px 间距、1px 点」）。颗粒墨色用与 `--lgi-ink` 完全相同的 RGB（17,19,21），只变透明度；α 是密度参数不是新颜色，选中态橙面颗粒同理。LIDS 的「不得写死间距值」按其字面在这两组上不成立：它们是材质自身的参数，不是布局间距。
- LIDS-MAT-001 关系（DECISION_REQUIRED）：`LIDS-MAT-001` 把六种材料建在**同一套 8px 采样点阵**上，并规定新增第七种材料要先证明它是同一点阵的第七种状态、再走治理；其材料预算还把「列表/正文」记为不使用纹理的白场。卡面颗粒是 20px 周期、偏移 13/6 的散点，**不在 8px 点阵的格点上**，按该文件的判据它不是既有六态之一。它的直接依据是 Mog 2026-09-15 的规格与选型（候选 B「采样残留（撒点）」、明确否掉方格纹理），而使它能成立的是该文件自己写的优先级：冲突时以用户最新确认为准。本包按用户指示实现，未把它登记为系统材料，也未改任何全局 token。若它应进入材料家族，两条路留 Mog 决定：①改为 8px 点阵的取样（24px 周期取 9 个格点中的 3 个，视觉近似不变，但像素会变）；②按 `design-governance.md` 把它登记为第七种材料。在此之前它只属于这张卡，不得当作可复用的系统材料。
...
- DECISION_REQUIRED：①卡面颗粒是否进入 `LIDS-MAT-001` 的材料家族（取值范围与两条路径见上面那条）；②结果区读取时间戳改挂列表容器后，悬停结果区任意位置都会浮出它（浏览器对没有自己 `title` 的后代取最近祖先的），是否保留这个可核对入口。用户已确认改动范围（含标题 CSS 一并修正）。
...
- 修改文件：`evidence_library.css`、`evidence_library.js`、`local_web.rs`（仅删除横条 HTML）、`tests.rs`（封面卡合同）；本清单、验收记录、设计索引和当月 progress。
- 已完成的验证（2026-09-15，1920×929 视口、检查器关闭、共 50 张卡）：
  ...
  - 断言回填验证：六个模板的每条坐标字面量都进了安全区合同，把 stack 的矩形从 `160×170` 改成 `260×260`（越出 x 48..252 / y 80..320）→ 只有该合同变红；把 `.ev-results-head` 的样式规则加回 CSS → 只有横条合同变红。CSS 断言覆盖的是独立 asset，所以横条合同同时负向断言 HTML 与 CSS 两侧。
