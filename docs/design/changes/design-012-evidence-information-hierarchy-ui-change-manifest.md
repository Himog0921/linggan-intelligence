...
> 状态: 权威当前
> 最后核对: 2026-09-03
> 适用范围: `/corpus/evidence` 的信息层级、LANG-05 Mono 预算迁移、边界声明收口，以及三处会让用户形成错误信念的缺陷
> 阅读提示: 本文档按七轮走查顺序追加。**「变更清单」记录首轮设计，其中若干项已被后续轮次修订或删除**；每处均在原位标注。最终形态以文末各轮「Mog 走查后的修订」为准。
...
> **本节的落地形态已被第三轮修订取代。** 顶部边界带于 Mog 走查后删除（理由见下文「第三轮」）。本节保留的是仍然成立的部分：区块内不再逐处渲染米色警告框，以及 `restricted` 与 `danger` 分色。
...
> **第 0 层「结论行」已被第二轮修订删除**（与材料完整度重复），其职责并入完整度区块。第 1–3 层仍然成立。

| 层 | 内容 | 默认 |
|---|---|---|
| ~~0 · 结论行~~ | ~~由真实 lane 状态复述~~ → 第二轮删除，并入完整度区块 | — |
...
| 边界带收口 | Inspector 内 `tone="warning"` 米色框数量由 5+ 降为 0（第三轮起顶部边界带亦删除） |
...
| 结论行（后已删除） | 当时实测「已取得 发现·媒体槽位·媒体字节·图片文字 / 尚未取得 详情·评论·回复·作者·视频转录」，与九个 lane cell 一致；第二轮并入完整度区块 |
| 边界带（后已删除） | 当时实测米色 `tone="warning"` 框归零、顶部一条边界带；第三轮该带亦删除 |
...
互动图标随后按 Mog 指定改为实心（`fill="currentColor"`，不再描边）。LIDS 图标系统在此处仍然成立的部分未变——SVG 而非字符字形、24 网格、`currentColor`、16px 档、带无障碍名；只有渲染方式从描边转为填充，因此 1.5 描边参数对这四个图标不再适用。

随后按 Mog 指定，表格里四个图标移到列头，行内只留数字并居中对齐：图标是在给列命名，逐行重复等于把同样的四件事说 46 遍。四列用 grid 与列头逐格对齐。行内的数字仍带 `aria-label`——脱离图标之后，一个裸数字说不出它数的是什么。列头图标由 JS 注入，与行内图标共用同一份 path 定义。

一条既有断言随之重锚：`channel?.requires` 原本守「复观测区块表达了授权条件」。删除常驻说明后，授权条件只在**不可执行**分支陈述（可执行即代表条件已满足），断言改为锚在那句原因说明上，守的不变量比原来更准。

## 材料完整度在表格中的处置（Mog 拍板方案 A）

Mog 问「材料 2/4 的价值跟意义是什么，告诉用户什么信息」。核实生产库后的答案：**在当前数据集里几乎没有信息**。

46 条中 43 条的详情通道同为 `UNKNOWN`，因而分数同为 `2/4`，区分度 6.5%（仅 `3/5`、`1/4`、`2/5` 三条不同）。指标本身回答的是真问题——这条素材够不够用——但它在两处失效：

1. **数据尚未分化**：43 条只跑过发现通道，五维状态天然一致。等这些作品跑过详情采集，分数才会分开。这是「还没到有意义的时候」，不是「指标无意义」。
2. **与相邻的状态列编码同一事实**：`2/4`↔`当前未知`、`3/5`↔`部分取得`。同一行里数字、进度条、状态词三个视觉元素说同一件事。

**处置（方案 A）**：表格中删除比值数字，只保留进度条，并把「状态」列并入材料列。7 列回到 6 列，材料列由 112px 加宽到 `minmax(176px,.8fr)`。

依据：三者之中进度条是**行与行之间形状会变化**的那一个，扫读时最容易发现「这行和别行不一样」；数字的精确性在扫读面上没有用武之地——判断「这条有没有正文」不需要知道是 2/4 还是 3/5。比值保留在检查器里，那里才是逐条评估单个作品的地方。
