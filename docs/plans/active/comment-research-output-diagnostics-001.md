# COMMENT-RESEARCH-OUTPUT-DIAGNOSTICS-001 · V4 结构输出诊断与候选绑定示例

> 状态: 活跃计划
> 最后核对: 2026-09-14
> 适用范围: Issue #258/#260 的 V4 评论研究输出合同、安全调用账本与受控真实运行
...
1. V4 输出合同进入研究指纹：同一评论不会把此前合同的失败或结论误认作 V4 的有效完成。
...
7. 部署后，按开发期授权重置**仅评论研究派生层**，再运行最多 20 条 V4 受控首批；只有全成功、发布 ResultRevision、五个读视图均可读时才扩大范围。
...
- Node：V4 packet 对 DeepSeek / OpenAI 已合格协议仍请求 named json schema。
- PostgreSQL：既有 Run、派生 reset、读取与未发布边界保持通过。
- Runtime：V4 受控批次用运行回执、发布状态、读取端点分别核验。
