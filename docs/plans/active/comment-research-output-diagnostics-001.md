...
评论研究运行失败时，用户可以知道失败发生在 JSON 传输、结构合同还是问题归并接纳阶段；系统不把原始评论、模型回复或模型思考写进常规账本，也不让部分失败 Run 生成统计 ResultRevision。已通过 membership 接纳的关系能否作为累计绝对事实读取，由后续 COMMENT-RESEARCH-CUMULATIVE-STATE-001 规定，不会放宽本卡的输出或接纳合同。
