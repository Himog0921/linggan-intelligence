...
Mog 已批准一个有界的 DESIGN-002 输入：以「任务启动困难」为例，制作 `SYNTHETIC / NOT LIVE` 的 Topic Intelligence Reference Page，用来走查 Topic 深入时的单一判断任务、视觉语言、状态诚实与组件晋升门。该交付物见 [`design-002-topic-intelligence-reference-page.md`](design-002-topic-intelligence-reference-page.md) 及其引用规格。
